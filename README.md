...
### حول الصورة الشخصية
تم استبدال فكرة الصورة الشخصية بعنصر تصميم دائم (حلقات رادار متحركة) يعكس طبيعة العمل، بحيث لا تحتاج أبدًا لصورة شخصية. إن رغبت لاحقًا بإضافة صورة حقيقية بدلًا منها، أخبرني وسأعيد تصميم هذا الجزء.
...
```
الشعارات تظهر بتدرّج رمادي وتتحول لألوانها الحقيقية عند تمرير الفأرة عليها تلقائيًا.

### لربط الفيديوهات بمعرض الأعمال
كل عنصر في مصفوفة `work` بداخل `content.js` فيه حقل `videoLink` — ضع فيه رابط فيديو يوتيوب (بأي صيغة: `youtube.com/watch?v=...` أو `youtu.be/...`). عند الضغط على الصورة المصغّرة، يبدأ الفيديو بالتشغيل مباشرة داخل نفس المساحة، دون أي نافذة منبثقة ودون تحويل الزائر إلى موقع يوتيوب.
